...
import { login, redirectToProvider, Flows } from '../api/allauth'
...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const resp = await login({ login: login_id, password })

      if (resp.status === 200 && resp.meta?.is_authenticated) {
        // JWT tokens are automatically extracted and stored by allauth.js
        navigate(from, { replace: true })
        ...
        const flow = resp.data.flows[0]
        if (flow?.id === Flows.VERIFY_EMAIL) {
          navigate('/account/verify-email')
        } else if (flow?.id === Flows.MFA_AUTHENTICATE) {
      ...
      <div className="auth_card">
        <div className="auth_header">
          ...
          <p>
            ...
            <Link to="/account/signup">sign up</Link> first.
          </p>
        </div>

        <form className="auth_form" id="login_form" onSubmit={handleSubmit}>
          {errors.length > 0 && (
